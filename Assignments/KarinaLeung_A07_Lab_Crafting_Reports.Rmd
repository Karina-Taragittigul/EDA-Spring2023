...
| Meta Data Item Name | Meta Data Values|
|--------------------:|:----------------|
...
```{r Boxplots, echo=FALSE, fig.align='left', fig.cap="Ozone Distribution by Month in 2018"}

Ozone_2018 <- epa_data %>% 
  filter(year(Date) == 2018)

ggplot(data=Ozone_2018, aes(x=factor(month(Date), levels = 1:12, labels = month.abb), y=Ozone)) +
  geom_boxplot() +
  labs(
    title ='Total Ozone in 2018',
    x = 'Month',
    y = expression(paste('Ozone Concentration(',mu,'g/m'^3,')')))

```

```{r Boxplots, echo=FALSE, fig.align='left', fig.cap="Ozone Distribution by Month in 2019"}

Ozone_2019 <- epa_data %>% 
  filter(year(Date) == 2019)

ggplot(data=Ozone_2019, aes(x=factor(month(Date), levels = 1:12, labels = month.abb), y=Ozone)) +
  geom_boxplot() +
  labs(
    title ='Total Ozone in 2019',
    x = 'Month',
    y = expression(paste('Ozone Concentration(',mu,'g/m'^3,')')))
