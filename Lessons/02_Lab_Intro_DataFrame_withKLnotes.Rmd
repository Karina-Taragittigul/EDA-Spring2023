...
```{r}
ncol(df) #tells you number of columns
nrow(df) #tells you number of rows
length(df)  #same as ncol
summary(df) #gives you summary statistics for each column
head(df) #show the first 6 rows of df; if head(df, n=#ofrows) then shows you that number of rows

#If you know the number of the column you want you can refer to that to access column
df[,1] #prints that column of the data set; nothing before the , tells R to print all rows and that one column
#df[1:10,1:2] tells R to show rows 1-10 and then columns 1-2; it's "df[rows,columns]"
