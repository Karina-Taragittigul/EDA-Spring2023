...
#the month() function is from lubridate

#another way to do this is
NTL.phys.data.Ex3 <- 
  NTL.phys.data %>%
  mutate(Month = month(sampledate)) %>% # Still using the lubridate column and this generates another column you                                             can filter from
...
NTL.phys.data.Ex4 <- 
  NTL.phys.data.Ex3 %>%
  filter(depth ==0) %>%
  drop_na(temperature_C) %>% # inside the pipe, use drop_na() over na.omit() because sometimes it doesn't work the                                 same way as the drop_na(). na.omit() doesn't drop observations, but rather is used                                 in tandem with another function because R cannot calculate when there are NAs in                                   the row/column
  summarize(meantemp = mean(temperature_C)) # the summarize() function will always give you less observations than                                                 when you started
```
