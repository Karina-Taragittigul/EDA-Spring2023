...
# Note: the droplevels() makes it so that the zero value lake names go away
...
# Note: is.na(column name) returns a logical output, so TRUE or FALSE
...
### rbind ; which means row bind, so combining data sets by rows
...
LitterFiles
# Note: list.files() function needs you to specify the path, and brings in all the files from that main path
Litter <- LitterFiles %>%
  plyr::ldply(read.csv)
# Note: reading every object on that 
```
