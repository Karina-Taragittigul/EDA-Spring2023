...
# Note: putting the fig.height and fig.width in the chunk heading automatically applies these settings to all plots generated throughout the chunk -- you can also just put them directly in the code

# Note: syntax of code is: ggplot(data frame, aesthetics(x = variable, y = variable)) +type of ggplot you want()
...
# Note: if you assign the ggplot to something, then you can create the plot object and then place that on your page wherever you want
# Note: syntax is a little different, aesthetics can either be specified in the ggplot or in the aesthetic layer
...
# Note: why didn't the code above work? When you specify color inside the aesthetics, it's like you're adding another layer to your plot rather than changing the color of the points. So, R reads it as if there's just one variable called "blue." But, we just want to change the color!

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)

# Note: To fix the code, you have to put the color OUTSIDE of the aesthetics
...
# Note: shape = as.factor(Year) will automatically assign different point shapes to the number of different options within the column. EX: 2018 and 2019 in Year, so two diff shapes for those. And color = Site.Name
...
# Note: facet_wrap generates sites in separate plots, so you don't need to separate the site.name by color like in the previous code
...
# Note: facet_grid(Site.Name by(~) year); has a column representing the year, and a row representing the three sites selected
